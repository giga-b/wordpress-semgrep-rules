<?php
/**
 * GamiPress Posts Restricted Shortcode
 *
 * @package     GamiPress\Restrict_Content\Shortcodes\Shortcode\GamiPress_Posts_Restricted
 * @since       1.0.8
 */
...
/**
 * Register the [gamipress_posts_restricted] shortcode.
 *
 * @since 1.0.8
 */
function gamipress_register_posts_restricted_shortcode() {

    $post_types = array_merge( array( 'all' => __( 'All', 'gamipress-restrict-content' ) ), gamipress_restrict_content_post_types() );

    gamipress_register_shortcode( 'gamipress_posts_restricted', array(
        'name'              => __( 'Posts Restricted', 'gamipress-restrict-content' ),
        'description'       => __( 'Renders a list of posts restricted.', 'gamipress-restrict-content' ),
        'icon'              => 'lock',
        'group'             => 'restrict_content',
        'output_callback'   => 'gamipress_posts_restricted_shortcode',
        'fields'      => array(
            'type' => array(
            ...
            'current_user' => array(
                ...
                'description' => __( 'Show only posts restricted to the current logged in user.', 'gamipress-restrict-content' ),
                'type' 		  => 'checkbox',
                'classes' 	  => 'gamipress-switch',
            ),
            'user_id' => array(
                'name'        => __( 'User', 'gamipress-restrict-content' ),
                'description' => __( 'Show only posts restricted of a specific user.', 'gamipress-restrict-content' ),
                'type'        => 'select',
                'classes' 	  => 'gamipress-user-selector',
                'default'     => '',
                'options_cb'  => 'gamipress_options_cb_users'
            ),
            'exclude_unlocked' => array(
                'name'          => __( 'Exclude posts unlocked', 'gamipress-restrict-content' ),
                'description'   => __( 'Exclude posts that user already got access.', 'gamipress-restrict-content' ),
                'type'          => 'checkbox',
                'classes'       => 'gamipress-switch'
            ),
            'orderby' => array(
...
add_action( 'init', 'gamipress_register_posts_restricted_shortcode' );
 ...
 */
function gamipress_posts_restricted_shortcode( $atts = array(), $content = '' ) {
    ...
    $atts = shortcode_atts( array(
        ...
        'order'                 => 'asc',
        'exclude'               => '',
    ), $atts, 'gamipress_posts_restricted' );
    ...
    // Exclude user unlocked posts
    if( $atts['exclude_unlocked'] === 'yes' )
        $exclude = array_merge( $exclude, gamipress_restrict_content_get_user_unlocked_posts( $atts['user_id'] ) );
    ...
    ob_start();
    gamipress_get_template_part( 'posts-restricted' );
    $output = ob_get_clean();
     ...
     * @param string    $content    Shortcode content
     */
    return apply_filters( 'gamipress_posts_restricted_shortcode_output', $output, $atts, $content );
}
