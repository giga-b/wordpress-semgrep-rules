...
function gamipress_leaderboards_get_user_position( $leaderboard_id, $user_id = null ) {
    ...
    if( $leaderboard_table->is_pagination_enabled() ) {
        ...
        if( (bool) $leaderboard_table->get_post_meta( 'cache' ) ) {
            ...
            // Fallback for paginated leaderboards without cache
            $leaderboard_table->query_vars['items_per_page'] = 0;
            $leaderboard_table->query_vars['max_items'] = 0;
            $leaderboard_table->query_vars['limit'] = '';
...
/**
 * Get the lowest priority rank of a rank type
 *
 * @since 1.4.3
 *
 ...
 * @return int
 */
function gamipress_leaderboards_get_lowest_priority_rank_menu_order( $rank_type = null ) {
    ...
    return apply_filters( 'gamipress_leaderboards_get_lowest_priority_rank_menu_order', absint( $menu_order ), $rank_type );
