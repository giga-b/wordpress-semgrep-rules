...
class GamiPress_Leaderboard_Table {
    ...
    public function __construct( $leaderboard_id = null, $args = array() ) {
        ...
        if( ! isset( $args['hide_no_earners'] ) ) {
            $args['hide_no_earners'] = (bool) $this->get_post_meta( 'hide_no_earners' );
        }
            ...
            if( in_array( $metric, $achievement_types ) ) {
                ...
                if( ! empty( $start_date ) || ! empty( $end_date ) ) {
                    ...
                    if( is_gamipress_upgraded_to( '6.9.4' ) ) {
                        $points_query = "
                            SELECT GREATEST( IFNULL( SUM( l{$i}.points ), 0 ), 0 )
                            FROM {$logs} AS l{$i}
                            WHERE l{$i}.user_id = u.ID
                              AND l{$i}.points != 0
                              AND l{$i}.points_type = '{$metric}'
                        ";
                    } else {
    ...
    public function get_items() {
        ...
        if( $this->results === false ) {
            ...
            // Excluded users with 0 earnings
            if( $this->args['hide_no_earners'] ) {

                // Setup vars
                $rank_types = gamipress_get_rank_types_slugs();
                $rank_priorities = array();
                $to_exclude = array();

                // Loop all results
                foreach( $this->results as $key => $result ) {

                    // Flag to meet if should be excluded
                    $should_exclude = true;

                    // Loop all metrics
                    foreach( $this->get_metrics() as $metric ) {

                        if( in_array( $metric, $rank_types ) ) {
                            // For ranks, we need to check if metric is different from lowest rank priority

                            if( ! isset( $rank_priorities[$metric] ) ) {
                                $rank_priorities[$metric] = gamipress_leaderboards_get_lowest_priority_rank_menu_order( $metric );
                            }

                            if( $result[$metric] > $rank_priorities[$metric] ) {
                                $should_exclude = false;
                            }
                        } else {
                            // For points and achievements simply check if is higher than 0
                ...
                if( count( $to_exclude ) > 0 ) {
                    // Update results
                    foreach( $to_exclude as $key ) {
                        unset( $this->results[$key] );
                    }

                    // Update found results (for pagination)
                    $this->found_results -= count( $to_exclude );
                }
    ...
    public function get_metrics() {
        ...
        $metrics_meta = $this->get_post_meta( 'metrics' );

        if( ! is_array( $metrics_meta ) ) {
            $metrics_meta = array();
        }

        $metrics = array();

        for ( $i=0; $i < count($metrics_meta); $i++ ) {
            if( ! in_array( $metrics_meta[$i], array( 'achievement_types', 'points_types', 'rank_types' ) ) ) {
                $metrics[] = $metrics_meta[$i];
            }
        }
