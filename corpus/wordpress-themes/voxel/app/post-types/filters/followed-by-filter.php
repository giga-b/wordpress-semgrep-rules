...
class Followed_By_Filter extends Base_Filter {
	...
	public function query( \Voxel\Post_Types\Index_Query $query, array $args ): void {
		...
		if ( $this->post_type->get_key() === 'profile' ) {
			$query->join( <<<SQL
			...
			$query->join( $wpdb->prepare( <<<SQL
				INNER JOIN {$wpdb->prefix}voxel_followers AS `{$followers_join_key}` ON (
					...
					AND `{$followers_join_key}`.follower_id = %d
					AND `{$followers_join_key}`.status = 1
					AND `{$followers_join_key}`.object_type = 'user'
					AND `{$followers_join_key}`.object_id = `{$posts_join_key}`.post_author
				)
			SQL, $user_id ) );
		} else {
			$query->join( $wpdb->prepare( <<<SQL
				INNER JOIN {$wpdb->prefix}voxel_followers AS `{$followers_join_key}` ON (
					...
					AND `{$followers_join_key}`.object_id = `{$query->table->get_escaped_name()}`.post_id
				)
			SQL, $user_id ) );
		}
	}
