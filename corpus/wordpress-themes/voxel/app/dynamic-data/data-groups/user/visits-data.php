		...
		return Tag::Object('Visit stats')->properties( function() {
			return [
				'views' => Tag::Object('Views')->properties( function() {
					return [
						'1d' => Tag::Number('Last 24 hours')->render( function() {
							return $this->user->stats->get_views('1d');
						} ),
						'7d' => Tag::Number('Last 7 days')->render( function() {
							return $this->user->stats->get_views('7d');
						} ),
						'30d' => Tag::Number('Last 30 days')->render( function() {
							return $this->user->stats->get_views('30d');
						} ),
						'all' => Tag::Number('All')->render( function() {
							return $this->user->stats->get_views('all');
						} ),
					];
				} ),
				'unique_views' => Tag::Object('Unique views')->properties( function() {
					return [
						'1d' => Tag::Number('Last 24 hours')->render( function() {
							return $this->user->stats->get_unique_views('1d');
						} ),
						'7d' => Tag::Number('Last 7 days')->render( function() {
							return $this->user->stats->get_unique_views('7d');
						} ),
						'30d' => Tag::Number('Last 30 days')->render( function() {
							return $this->user->stats->get_unique_views('30d');
						} ),
						'all' => Tag::Number('All')->render( function() {
							return $this->user->stats->get_unique_views('all');
						} ),
					];
				} ),
				'countries' => Tag::Object_List('Top countries')->items( function() {
					return $this->user->stats->get_tracking_stats('countries');
				...
				'ref_domains' => Tag::Object_List('Top referrers (domains)')->items( function() {
					return $this->user->stats->get_tracking_stats('ref_domains');
				} )->properties( function( $index, $item ) {
					return [
				...
				'ref_urls' => Tag::Object_List('Top referrers (URLs)')->items( function() {
					return $this->user->stats->get_tracking_stats('ref_urls');
				} )->properties( function( $index, $item ) {
					return [
				...
				'browsers' => Tag::Object_List('Top browsers')->items( function() {
					return $this->user->stats->get_tracking_stats('browsers');
				} )->properties( function( $index, $item ) {
					return [
				...
				'platforms' => Tag::Object_List('Top platforms')->items( function() {
					return $this->user->stats->get_tracking_stats('platforms');
				} )->properties( function( $index, $item ) {
					return [
				...
				'devices' => Tag::Object_List('Devices')->items( function() {
					return $this->user->stats->get_tracking_stats('devices');
				} )->properties( function( $index, $item ) {
					return [
				...
				'last_updated' => Tag::Date('Last updated')->render( function() {
					$time = $this->user->stats->get_last_updated_time();
					if ( $time === null ) {
						return '';
					}
