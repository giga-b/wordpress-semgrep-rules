...
export class ProblemProvider implements vscode.TreeDataProvider<ProblemItem> {
    ...
    getChildren(element?: ProblemItem): Thenable<ProblemItem[]> {
        ...
        if (element.contextValue === 'file') {
            ...
            return Promise.resolve(fileProblems.map(p => new ProblemItem(
                `${this.getSeverityIcon(p.severity)} ${p.message}`,
    ...
    private formatSeverityCounts(counts: { [key: string]: number }): string {
        ...
        if (counts.warning > 0) parts.push(`⚠️ ${counts.warning} warning(s)`);
        if (counts.info > 0) parts.push(`ℹ️ ${counts.info} info`);
        return parts.join(', ');
    }
...
export class ProblemItem extends vscode.TreeItem {
    constructor(
        ...
        if (problem) {
            ...
            this.iconPath = this.getIconPath(problem.severity);
            this.command = {
                command: 'vscode.open',
                title: 'Open File',
                arguments: [
    ...
    private createTooltip(problem: SemgrepResult): string {
        let tooltip = `**${problem.severity}**: ${problem.message}\n\n`;
        tooltip += `**File**: ${problem.path}\n`;
        tooltip += `**Line**: ${problem.start.line}\n`;
        tooltip += `**Rule**: ${problem.check_id}\n`;
        
        if (problem.metadata?.category) {
            tooltip += `**Category**: ${problem.metadata.category}\n`;
        }
        
        if (problem.metadata?.cwe) {
            tooltip += `**CWE**: ${problem.metadata.cwe}\n`;
        }
        
        if (problem.metadata?.references && problem.metadata.references.length > 0) {
            tooltip += `**References**:\n${problem.metadata.references.map(ref => `- ${ref}`).join('\n')}`;
        }
