<?php
/**
 * Advanced Vulnerable Nonce Test Cases
 * - Missing checks, wrong fields, variable actions, default param misuse, etc.
 */

// Lightweight WP function stubs for linting in isolated test context
if (!function_exists('add_action')) { function add_action($hook, $callback, $priority = 10, $args = 1) {} }
if (!function_exists('update_option')) { function update_option($key, $value) { return true; } }
if (!function_exists('check_ajax_referer')) { function check_ajax_referer($action, $name = '_ajax_nonce', $die = true) { return true; } }
if (!function_exists('wp_nonce_field')) { function wp_nonce_field($action, $name = '_wpnonce') { echo '<input type="hidden" name="' . $name . '" value="nonce" />'; } }
if (!function_exists('wp_verify_nonce')) { function wp_verify_nonce($nonce, $action) { return 1; } }
if (!function_exists('wp_nonce_url')) { function wp_nonce_url($url, $action) { return $url . (strpos($url, '?') === false ? '?' : '&') . '_wpnonce=nonce'; } }
if (!function_exists('admin_url')) { function admin_url($path = '') { return '/wp-admin/' . ltrim($path, '/'); } }
if (!function_exists('esc_url')) { function esc_url($url) { return $url; } }
if (!function_exists('check_admin_referer')) { function check_admin_referer($action) { return true; } }
if (!function_exists('wp_enqueue_script')) { function wp_enqueue_script() {} }
if (!function_exists('plugins_url')) { function plugins_url($file, $ref) { return '/plugins/' . basename($file); } }
if (!function_exists('wp_localize_script')) { function wp_localize_script() {} }
if (!function_exists('wp_create_nonce')) { function wp_create_nonce($action) { return 'nonce'; } }
if (!function_exists('wp_send_json_success')) { function wp_send_json_success($data) { echo json_encode(array('success' => true, 'data' => $data)); } }

// VULN: admin-post handler without nonce verification
add_action('admin_post_my_insecure_action', 'wprs_vuln_admin_post_handler');
function wprs_vuln_admin_post_handler() {
    $data = $_POST['data'] ?? '';
    update_option('wprs_vuln_admin_post', $data);
}

// VULN: admin-post (nopriv) without nonce verification
add_action('admin_post_nopriv_my_insecure_action', 'wprs_vuln_admin_post_nopriv');
function wprs_vuln_admin_post_nopriv() {
    $data = $_POST['data'] ?? '';
    update_option('wprs_vuln_admin_post_nopriv', $data);
}

// VULN: AJAX with check_ajax_referer using generic/empty action
add_action('wp_ajax_wprs_vuln_default_param', function () {
    check_ajax_referer('action');
    $val = $_POST['val'] ?? '';
    update_option('wprs_vuln_default_param', $val);
});

// VULN: Custom nonce field name but wrong verification action
function wprs_vuln_custom_nonce_field_render() {
    ?>
    <form method="post">
        <?php wp_nonce_field('wprs_custom_action', '_wprs_nonce'); ?>
        <input type="text" name="foo" />
        <input type="submit" value="Save" />
    </form>
    <?php
}

function wprs_vuln_custom_nonce_field_process() {
    if (!empty($_POST['foo'])) {
        // Wrong action used for verification
        if (wp_verify_nonce($_POST['_wprs_nonce'] ?? '', 'different_action')) {
            update_option('wprs_vuln_custom_nonce', $_POST['foo']);
        }
    }
}

// VULN: URL nonce created but verified with wrong action
function wprs_vuln_url_nonce_link() {
    $url = wp_nonce_url(admin_url('admin.php?page=wprs'), 'wprs_url_action');
    echo '<a href="' . esc_url($url) . '">Go</a>';
}

add_action('admin_init', function () {
    if (isset($_GET['page']) && $_GET['page'] === 'wprs' && isset($_GET['_wpnonce'])) {
        // Wrong referer action
        check_admin_referer('different_action');
    }
});

// VULN: Localized script nonce not verified server-side
function wprs_vuln_enqueue() {
    wp_enqueue_script('wprs-vuln', plugins_url('wprs-vuln.js', __FILE__), array('jquery'), '1.0', true);
    wp_localize_script('wprs-vuln', 'WPRS_VULN', array(
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce'    => wp_create_nonce('wprs_localized_action')
    ));
}

add_action('wp_ajax_wprs_localized_vuln', function () {
    // Missing check_ajax_referer
    wp_send_json_success(array('ok' => true));
});

// VULN: OO callback without proper nonce verification
class WPRS_Vuln_Handler {
    public static function init() {
        add_action('wp_ajax_wprs_oo_vuln', array(__CLASS__, 'handle'));
    }

    public static function handle() {
        // Missing nonce verification
        $msg = $_POST['msg'] ?? '';
        wp_send_json_success(array('msg' => $msg));
    }
}

WPRS_Vuln_Handler::init();


